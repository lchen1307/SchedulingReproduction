...
class MCTP:
    ...
    def readData(self, filename):
        with open(filename, 'r') as data:
            self.NumOrg = int(data.readline().split()[0])
            self.NumDes = int(data.readline().split()[0])
            self.NumProduct = int(data.readline().split()[0])

            for i in range(self.NumOrg):
                valid_line_read = False
                while not valid_line_read:
            ...
            for i in range(self.NumOrg):
                ...
                CapacityData = []
                for j in range(self.NumDes):
                    CapacityData.append(float(col[j]))
    ...
    def optimizeFinalRMP(self):
        ...
        opt_x = []
        for i in range(self.NumOrg):
            opt_x_commodity = [0.0] * self.NumDes
